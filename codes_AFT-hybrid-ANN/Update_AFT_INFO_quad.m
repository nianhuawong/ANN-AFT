function [AFT_stack_sorted, nCells_AFT]= Update_AFT_INFO_quad(AFT_stack_sorted, node1_base, node2_base, node_select, nCells_AFT , xCoord_AFT, yCoord_AFT)
global cellNodeTopo nCells_quad;
cellNodeTopo(end+1,:) = [node1_base, node2_base, node_select(2), node_select(1)];
...
[row, direction] = FrontExist(node2_base, node_select(2), AFT_stack_sorted);  %判断阵面（node_select，node1_base）是否已经存在及其在AFT_stack_sorted中的方向
...
[row, direction] = FrontExist(node_select(2), node_select(1), AFT_stack_sorted); 
...
[row, direction] = FrontExist(node_select(1), node1_base, AFT_stack_sorted); 
