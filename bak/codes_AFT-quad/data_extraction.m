...
Sp          = 0.6;      % 网格步长  % Sp = sqrt(3.0)/2.0;  %0.866
al          = 3.0;      % 在几倍范围内搜索
...
while size(AFT_stack_sorted,1)>0
    ...
    for ii = 1:2
        ...
        for i = 1 : length(nodeCandidate)
            ...
            for j = 1:length(node_test_list)
                ...
                if flagNotCross == 1 && flagLeftCell == 1 && flagSpecial == 1
    ...
    if quality < 0.8        
        xCoord_AFT = xCoord_AFT(1:end-sum(flag_best));
        yCoord_AFT = yCoord_AFT(1:end-sum(flag_best));
        ...
        for i = 1 : length(nodeCandidate)
            ...
            for j = 1:length(node_test_list)
                node_test = node_test_list(j);
                flagNotCross = IsNotCross(node1_base, node2_base, node_test, frontCandidate, AFT_stack_sorted, xCoord_tmp, yCoord_tmp ,0);
                flagLeftCell = IsLeftCell(node1_base, node2_base, node_test, xCoord_tmp, yCoord_tmp);
    ...
    if outGridType == 0 && length(node_select) == 2    
